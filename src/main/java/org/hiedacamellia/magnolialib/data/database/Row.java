...
import org.hiedacamellia.magnolialib.util.Patterns;
...
public class Row {
    ...
    Row(Table table, String[] labelset, String[] dataset) {
        try {
            ...
            MagnoliaLib.LOGGER.error("Failed to set the values for the label set: " + Arrays.toString(labelset) +
    ...
    public <T> T get(String label) {
        if (!data.containsKey(label.toLowerCase(Locale.ENGLISH))) {
            MagnoliaLib.LOGGER.error("Failed to find the label: " + label + " in the row: " + this + " of the table: " + table.name());
            return null;
        }
