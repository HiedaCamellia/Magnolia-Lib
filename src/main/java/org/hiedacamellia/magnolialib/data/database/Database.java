package org.hiedacamellia.magnolialib.data.database;
...
public class Database implements ResourceManagerReloadListener {
    ...
    private static final int dirLength = MagnoliaLib.DATABASE_FOLDER.length() + 1;
...
//        }
    ...
    public static Table loadTable(@Nonnull ResourceManager rm, String table) {
        ...
        rm.listResources(MagnoliaLib.DATABASE_FOLDER, (fileName) -> fileName.toString().endsWith(".csv")).forEach((rl, rs) -> loadData(tables, rl, rs));
    ...
    public void onResourceManagerReload(@NotNull ResourceManager rm) {
        ...
        rm.listResources(MagnoliaLib.DATABASE_FOLDER, (fileName) -> fileName.toString().endsWith(".csv"))
