...
public abstract class GhostBlockRenderer<D, T extends GhostBlockGetter<D>> implements AutoCloseable {
    ...
    private void setupGhostRenderer() {
        ...
        RenderType.chunkBufferLayers().forEach(renderType -> new BufferBuilder(newBuffers.buffer(renderType),renderType.mode(),renderType.format()));
        ...
        for (final RenderType renderType : renderTypes) {
            MeshData newBuffer = new BufferBuilder(newBuffers.buffer(renderType),renderType.mode(),renderType.format()).build();
            if (newBuffer == null) {
    ...
    protected void addRender(SectionBufferBuilderPack newBuffers, BlockRenderDispatcher blockRenderer, PoseStack poseStack, BlockState state, BlockPos pos, RandomSource random) {
        ...
        try {
            if (!fluidState.isEmpty()) {
                ...
                VertexConsumer buffer = GhostlyVertexConsumer.of(new BufferBuilder(newBuffers.buffer(renderType),renderType.mode(),renderType.format()), chunkOffsetX, chunkOffsetY, chunkOffsetZ);
            ...
            if (state.getRenderShape() != RenderShape.INVISIBLE) {
                ...
                for (RenderType renderType : model.getRenderTypes(state, random, modelData)) {
                    renderBatched(blockRenderer, state, pos, blockGetter, poseStack, new BufferBuilder(newBuffers.buffer(renderType),renderType.mode(),renderType.format()), random, modelData, renderType);
